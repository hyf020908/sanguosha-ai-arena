...
`sanguosha-ai-arena` 是一个“人与 AI 玩三国杀”的开源项目。当前 v0.2 实现了完善标准游戏牌（108张）、后端合法动作约束和 AI 决策提示词，不加入武将、英雄技能或官方素材。
...
<div align="center">
  <img src="./assets/5.png" alt="项目截图 5" width="720" />
</div>
...
- 已实现杀、闪、桃、无懈可击、南蛮入侵、万箭齐发、决斗、无中生有、桃园结义、五谷丰登、过河拆桥、顺手牵羊、借刀杀人、乐不思蜀、闪电等所有牌型。
- 已实现装备区、判定区、攻击范围、坐骑距离、诸葛连弩、青釭剑、八卦阵、仁王盾等无武将技能版本下的基础效果。
- 所有出牌、响应、弃牌和装备动作都绑定具体 `card_id`，同名不同花色/点数的牌会生成不同动作。
- 过河拆桥、顺手牵羊可选择随机手牌、公开装备区牌或公开判定区牌；五谷丰登使用公共牌池逐人选择。
- 支持延时锦囊进入判定区前的无懈可击窗口、无懈可击反无懈链、借刀杀人的目标选择，以及后放先判的判定区顺序。
- 支持主公/反贼/内奸胜利判定，以及击杀反贼摸 3 张、主公误杀忠臣弃置所有手牌和装备。
...
- 将版本升级到 v0.2。
- 把最近事件改为倒序排列，最上面显示最新的事件
- 将牌堆从 55 张基础牌扩展为 108 张标准版游戏牌。
- 新增锦囊、延时锦囊、装备牌、距离和攻击范围。
- 新增无懈可击响应、群体锦囊逐目标结算、完整濒死求桃、装备区和判定区公开展示。
- 新增具体 `card_id` 动作、中文牌名 + 花色图标 + 点数展示、五谷丰登公共牌池、无懈可击链式反制、借刀杀人响应选择。
- 修复闪电移动复制、闪电命中跳过摸牌、延时锦囊无懈窗口、青釭剑误影响万箭齐发、过河拆桥可对自己使用等规则问题。
- 完善身份胜负和死亡奖惩：内奸可单独胜利，击杀反贼摸 3 张，主公杀忠臣弃置所有手牌和装备。
...
如果你的默认 `python` 不是 Python 3.10+，请把下面命令中的 `python` 替换为 `python3.11` 或其他符合要求的解释器。
...
- 继续校准标准规则细节，例如更多武器特效、连环/属性伤害、复杂多人目标选择和更细的结算时机。
- 反复迭代模型推理效果、身份推理和阵营策略。
- 增加更多前端交互细节，例如更清晰的响应链展示、牌池选择高亮和复盘视图。
- 在后续大版本加入英雄选择和武将技能。
