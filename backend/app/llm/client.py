...
class LLMClient:
    async def choose_action(
        ...
        for attempt in range(2):
            ...
            try:
                ...
                if action_id in legal_ids:
                    logger.info(
                        "AI action request success: ai=%s model=%s attempt=%s action_id=%s latency_ms=%s",
                        config.name,
                        config.model,
                        attempt_label,
                        action_id,
                    ...
                    return action_id
                logger.warning(
                    "AI returned illegal action_id: ai=%s model=%s attempt=%s action_id=%r legal_action_ids=%s latency_ms=%s raw_content=%s",
                    config.name,
                    config.model,
                    attempt_label,
                    action_id,
                ...
                logger.warning(
                    "AI HTTP error: ai=%s model=%s url=%s attempt=%s status=%s latency_ms=%s response=%s",
                    config.name,
                    config.model,
                    url,
                    attempt_label,
                ...
                logger.warning(
                    "AI network error: ai=%s model=%s url=%s attempt=%s error_type=%s latency_ms=%s error=%s",
                    config.name,
                    config.model,
                    url,
                    attempt_label,
                ...
                logger.warning(
                    "AI JSON parse error: ai=%s model=%s url=%s attempt=%s latency_ms=%s error=%s raw_content=%s",
                    config.name,
                    config.model,
                    url,
                    attempt_label,
                ...
                logger.warning(
                    "AI response schema error: ai=%s model=%s url=%s attempt=%s error_type=%s latency_ms=%s error=%s response=%s",
                    config.name,
                    config.model,
                    url,
                    attempt_label,
                ...
                logger.exception(
                    "AI unexpected error: ai=%s model=%s url=%s attempt=%s error_type=%s latency_ms=%s error=%s",
                    config.name,
                    config.model,
                    url,
                    attempt_label,
        ...
        logger.warning(
            "AI action request exhausted retries: ai=%s model=%s url=%s latency_ms=%s",
            config.name,
            config.model,
            url,
            _latency_ms(total_started_at),
        )
        return None
