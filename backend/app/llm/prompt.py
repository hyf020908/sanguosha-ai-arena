...
SYSTEM_PROMPT = (
    ...
    "Save allies in dying state with Tao when it helps your camp, but hidden roles are not known: infer likely allies and enemies from recent_events; loyalists know the public lord and must protect the lord. "
...
CARD_RULES = {
    "core_contract": {
        ...
        "camp_advantage": "每个动作都按阵营收益评估：伤害疑似敌方、削弱疑似敌方装备/手牌、保护公开核心或疑似队友、救疑似队友、扩大自己后续攻击能力都属于正收益；伤害疑似队友、给疑似敌方回血/摸牌、浪费强牌属于负收益。unknown 不是已知阵营，必须通过 recent_events 推断。",
    },
    "turn_flow": {
    ...
    "basic": {
        ...
        "tao": "桃：出牌阶段只能对自己使用，且自己必须已受伤；回复 1 点体力且不超过体力上限。其他角色受伤但未濒死时，不能主动对其使用桃。濒死求桃时，被请求角色如果有桃，可以选择对濒死角色使用桃。救不救别人应根据公开身份和 recent_events 推断阵营，而不是假设知道隐藏身份。AI 策略：自己体力 <=2、濒死或能避免马上死亡时高度优先；忠臣必须救公开主公；反贼根据日志推断疑似反贼同伴并积极救援，通常不救公开主公；主公谨慎救疑似忠臣，不要救明显持续压主公的人；内奸优先自保并维持局势平衡。",
    },
    "trick": {
        ...
        "shunshouqianyang": "顺手牵羊：对距离 1 以内且有牌的其他角色使用，获得其 1 张牌。AI 策略：这是强收益牌，应积极使用；优先顺疑似敌方低手牌、关键装备或公开主公/疑似反贼核心资源；不要顺明显疑似己方核心防御。选择区域时：能拿到疑似敌方武器、防具、马并立即改善自己进攻/防御时优先拿公开装备；疑似敌方手牌少且可能握有闪/桃/杀/无懈时随机手牌价值高；不要拿走疑似队友保命装备，除非该动作唯一能防止马上输。",
        ...
        "wugufengdeng": "五谷丰登：结算时先按存活角色数亮出同数量公共牌，从使用者开始按座次每名存活角色依次选择一张获得；所有人选择后剩余牌进弃牌堆。AI 策略：选择牌时必须根据自己的身份、体力、手牌、装备、距离和当前局势选择最有利的具体 card_id。低血或可能濒死优先桃；缺防御且会被攻击优先闪/八卦阵/仁王盾/+1马；能立即进攻公开主公或疑似敌方核心时优先杀、决斗、拆顺、乐不思蜀、武器或-1马；已有杀但距离不够优先武器或-1马；需要保护疑似队友或阻止疑似敌方收益时优先无懈可击；不要机械选择第一张牌，也不要只看牌面强度而忽视阵营目标。",
    ...
    "targeting_policy": {
        "zhu": "主公：优先生存。主公不知道其他隐藏身份，只能根据行为猜忠臣、反贼、内奸：攻击主公、拆顺主公、无懈主公收益、救明显压主公者的人更像反贼；救主公、无懈伤害主公的锦囊、攻击疑似反贼的人更像忠臣；长期两边削弱、保守自保、避免明显站队的人可能是内奸。攻击疑似反贼，保护疑似忠臣，警惕内奸坐收残局。",
        "zhong": "忠臣：保护公开主公是最高优先级。不要主动伤害主公，不要使用会伤害主公的群体牌；优先攻击疑似反贼或伤害过主公的人。根据日志找反贼：谁持续压主公、帮压主公的人、无懈主公收益，谁更像反贼。忠臣知道主公身份，但其他 unknown 玩家仍必须靠行为推断。",
    ...
    "response_policy": {
        ...
        "dying_tao": "濒死求桃时，救自己永远优先。忠臣看到公开主公濒死且自己有桃，必须出桃救。救其他 unknown 玩家时不能假设知道其真实身份：反贼应根据 recent_events 判断谁像反贼同伴，压主公明显、无懈主公收益、攻击主公者更值得救；主公和忠臣应根据 recent_events 判断谁像忠臣，救保护主公者，通常不救持续压主公的人；内奸按生存和平衡判断，避免任一阵营过早获胜。但如果 valid_action_ids 没有完整 dying_tao:<card_id>，就代表当前没有桃可用，不能幻想使用桃。",
        "wuxie": "无懈可击只用于抵消当前锦囊效果。不要保守囤无懈。看到疑似敌方获得明显收益、己方关键角色被压制、公开主公被伤害或控制、疑似队友被关键控制时，应主动使用。反贼尤其要压制公开主公的摸牌、回血、偷牌和控制；忠臣尤其要保护公开主公不受伤害和控制。对 unknown 玩家必须结合 recent_events 推断，不要当成已知阵营。但如果 valid_action_ids 没有完整 wuxie:<card_id>，就不能返回 wuxie。",
    },
    "wuxie_examples": {
        "rebel_should_wuxie": "你是反贼，公开主公使用无中生有、顺手牵羊、桃园结义给自己回血、五谷丰登获得牌，或对疑似反贼同伴使用乐不思蜀/过河拆桥/顺手牵羊/决斗时，如果 legal_actions 有 wuxie，通常应选择 wuxie。疑似同伴必须从 recent_events 推断。",
        "loyalist_should_wuxie": "你是忠臣，疑似反贼对公开主公使用过河拆桥、顺手牵羊、决斗、乐不思蜀，或南蛮/万箭会伤害主公时，如果 legal_actions 有 wuxie，通常应选择 wuxie。",
        "should_not_wuxie": "不要无懈己方明显收益：忠臣不要无懈主公的无中生有；反贼不要无懈疑似反贼同伴压制主公的锦囊；受益目标不要无懈自己的收益。unknown 不能当作已知同伴。",
    },
    "camp_win_strategy": {
        "zhu": [
            ...
            "濒死求桃时必须自救；保护过主公、像忠臣的 unknown 濒死且救他能维持主公阵营优势时应救。",
        ],
        "zhong": [
            ...
            "主公濒死且你有桃时必须救；其他 unknown 玩家只有在 recent_events 显示其像忠臣、且不会害死主公时才应救。",
        ],
        "fan": [
            ...
            "疑似反贼同伴濒死且你有桃时通常必须救，因为反贼人数就是压主公的资源；但同伴身份必须由 recent_events 推断，不能把 unknown 当作已知反贼。",
        ],
        "nei": [
    ...
    "action_priority": [
        ...
        "濒死求桃：有桃时不要机械 pass。先判断濒死者是否是自己、公开主公或根据 recent_events 推断出的疑似队友；疑似队友通常要救，疑似敌人通常不救。",
...
def compact_prompt(state: GameState, player: Player, legal_actions: list[Action]) -> dict:
    ...
    return {
        ...
        "aggressive_strategy": [
            ...
            "有合法杀且目标不违反身份禁忌时，优先出杀；反贼能杀公开主公时必须优先主公，主公和忠臣有杀且能打疑似反贼时必须积极出杀。",
        ...
        "decision_guidance": [
            ...
            "使用桃和无懈可击时按公开身份和 recent_events 推断阵营收益：该救疑似队友就救，该阻止疑似敌方收益就阻止，该反无懈保护己方关键锦囊就反无懈。",
